    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if response.status_code == 202:
                ...
                # Poll the operation_location until processing is done
                while True:
                    time.sleep(POLLING_INTERVAL)
                    result_response = requests.get(operation_location, headers={"Ocp-Apim-Subscription-Key": "2fe1b91a80f94bb2a751f7880f00adf6"})
                    result_data = result_response.json()

                    if result_data.get("status") not in ["notStarted", "running"]:
